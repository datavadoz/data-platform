...
resource "google_cloud_run_v2_job" "monitor_traffic_dev" {
  project  = module.prj_conda_cps_dev.project_id
...
resource "google_cloud_scheduler_job" "monitor_traffic_dev" {
  project          = module.prj_conda_cps_dev.project_id
  name             = "monitor-traffic"
  region           = var.region
  ...
  http_target {
    http_method = "POST"
    uri         = "https://${var.region}-run.googleapis.com/apis/run.googleapis.com/v1/namespaces/${module.prj_conda_cps_dev.project_id}/jobs/${google_cloud_run_v2_job.monitor_traffic_dev.name}:run"
